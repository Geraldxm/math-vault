...
该 benchmark 收录 Google Minerva 评测使用的 MIT OpenCourseWare 定量题，覆盖 9 门大学 STEM 课程。上游只发布名为 `train` 的单一 split，本目录保持该命名，不据此推断训练用途。

答案位于解答文本末端的 LaTeX `\boxed{}` 中，包含数值、分数、表达式与单位。评测时应使用支持嵌套 box、LaTeX 等价和单位处理的 parser。
