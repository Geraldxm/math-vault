...
- 领域: MATH benchmark 子集；7 个科目
- 难度: `level` 1–5
- 答案格式: 字符串；混合整数、十进制数、LaTeX 表达式、方程、区间/元组与文本答案
- 推荐解析方式: 评测 prompt 要求最终答案放入 `\boxed{}`；提取最后一个 box 并支持嵌套花括号，再根据 gold 类型分别解析数值、LaTeX、方程、集合/区间或文本并做等价判定。逗号也可能是元组、区间或千位分隔，不能直接切分。参考 [Hugging Face Math-Verify](https://github.com/huggingface/Math-Verify)
