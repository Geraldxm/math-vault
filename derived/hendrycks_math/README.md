...
本层为训练适配和审计提供 `id/problem/answer/answer_status/solution/subject/level/level_raw/source`；两条上游 `Level ?` 记录的 `level=null`、原值保留于 `level_raw`。上游有两条空 `\\boxed{}`，其 `answer=null`、`answer_status=missing_boxed_answer`，不由本层猜测补全。论文复现训练只消费 `train.jsonl`；`test.jsonl` 不得进入训练。
